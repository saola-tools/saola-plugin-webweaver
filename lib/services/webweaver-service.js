...
var Service = function(params) {
  ...
  self.getJsonBodyParserLayer = function(branches, path) {
    jsonBodyParser = jsonBodyParser || bodyParser.json({
      limit: webweaverConfig.jsonBodySizeLimit || '2mb'
    });
    return {
  ...
  self.build = function(layerOrBranches, pathOfBranches) {
    if (lodash.isArray(layerOrBranches)) {
      var path = pathOfBranches;
      if (path) {
        if (!(lodash.isArray(path) && lodash.isEmpty(path))) {
          if (debugx.enabled) {
            var p = lodash.isString(path) ? path : JSON.stringify(path);
            debugx(' - layerware for path: %s', p);
          }
          apporo.use(path, buildMiddlewares(layerOrBranches));
        }
      } else {
        debugx.enabled && debugx(' - layerware for any request');
        apporo.use(buildMiddlewares(layerOrBranches));
      }
    } else {
      if (lodash.isObject(layerOrBranches)) {
        apporo.use(buildMiddlewares([layerOrBranches]));
      }
    }
  }
